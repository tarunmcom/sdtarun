...
def start_training():
    args = request.json
    required_args = ["job_id", "unique_user_id", "project_name", "s3_bucket", "s3_folder", "person_name"]
    ...
    if not re.match(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', args["unique_user_id"]):
        return jsonify({"error": "Invalid unique_user_id. Must be a valid UUID."}), 400
